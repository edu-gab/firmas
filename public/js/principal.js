$(document).ready(async function() {
    ...
    let table = $("#firmas").DataTable({
        ...
        buttons: [
            {
                extend:    'excelHtml5',
                text:      `<img src="https://${window.location.host}/berlinGestion/public/images/icons/dataTable/archivo-xls.png" alt="Hombre" width="32" height="32">`,
                titleAttr: 'Excel',
                title: 'Excel_export_'+hoy.toISOString(),
                messageBottom: 'Documento generado por TECNOVA S.A. el '+hoy.toUTCString(),
                exportOptions: {
                    columns: [ 0, 1, 2],
                    orthogonal: `export`
                }
            ...
            {
                extend:    'pdf',
                text:      `<img src="https://${window.location.host}/berlinGestion/public/images/icons/dataTable/archivo-pdf.png" alt="Hombre" width="32" height="32">`,
                titleAttr: 'PDF',
                title: 'PDF_export_'+hoy.toISOString(),
                messageTop: 'Documento generado por TECNOVA S.A. el '+hoy.toUTCString(),
                download: 'open',
            ...
            {
                extend:    'print',
                text:      `<img src="https://${window.location.host}/berlinGestion/public/images/icons/dataTable/imprimir.png" alt="Hombre" width="32" height="32">`,
                titleAttr: 'Imprimir',
                messageTop: 'Documento generado por TECNOVA S.A.',
                exportOptions: {
                    columns: [ 0, 1, 2],
                    orthogonal: `export`
                }
        ...
        "columns": [
            ...
            {data: "extension"},
            {
                ...
                render: function (data, type, row) {
                    if (type === 'display') {
                        return `<td><img data-search="${data === '1' ? 'con firma' : 'sin firma'}" src="${base_url}/public/images/icons/${data === '1' ? 'valido' : 'invalido'}.png" alt="Asignado" width="24" height="24"></td>`
                    } else if (type === 'filter') {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return `<td><img class="${data.tiene_firma === '1' ? 'my-btn' : 'btn-disabled'}" src="${base_url}/public/images/icons/enlace.png" ${data.tiene_firma === '1' ? funcion : ''} alt="FirmaLink"/></td>`;
                },
                orderable: false,
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return `<td><img class="${data.tiene_firma === '1' ? 'my-btn' : 'btn-disabled'}" src="${base_url}/public/images/icons/email.png" ${data.tiene_firma === '1' ? funcion : ''} alt="FirmaEnvio"/></td>`;
                },
                orderable: false,
            },
            {
    ...
    setTimeout( () => {
        ...
        $('#firmas_info, #firmas_length, #firmas_paginate, .pagination').addClass("fuente");
    }, 400)

    $("#opcionForm").on('submit', ( async function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        if((filename === null || filename === undefined) && (imagen === null || imagen === undefined)) {
        ...
        $('#cargar').html(`<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Enviando...`)
            .prop("disabled", true);
            ...
            .then((response)=> {
                ...
                response.msg.forEach(rmsg => {
                    if(rmsg.msg.length !== 0) {
                        HTMLresponse = HTMLresponse + `<div><img data-search="${rmsg.isSuccess? 'con firma' : 'sin firma'}" src="${base_url}/public/images/icons/${rmsg.isSuccess? 'valido' : 'invalido'}.png" alt="Estado" width="24" height="24">`;
...
function getURL(link) {
    ...
    let estilo = document.createTextNode('#aviso {position:fixed; z-index: 9999999; top: 80%;left:50%;margin-left: -70px;padding: 20px; background: #C7C5C5;border-radius: 8px;font-family: sans-serif;opacity:0.75;}');
...
async function fillModal(empleado)
{
    ...
    if (empleado.tiene_firma === '1') {
        ...
        $('#firmaModal #firmaImage').attr('src', `https://webapps.boschecuador.com/firmas/${empleado.archivo}`);
...
{
    Swal.fire({
        title: '¿Está seguro de enviar el correo de configuración para este colaborador?',
...
{
    Swal.fire({
        title: '¿Está seguro de enviar el correo de solicitud de firma para este colaborador?',
        text: "Esta acción no se podrá revertir.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
...
function cleanModal()
{
    ...
    $('#firmaModal #ext').val('');
    $('#firmaModal #firmaImage').attr('src', `https://res.cloudinary.com/dtutqsucw/image/upload/v1438955603/file-upload-01.png`);
...
function inicialAlert(response = null)
{
    swal.fire({
        ...
        text: response.status===true?`${response.msg}, nuevos colaboradores: ${response.nuevos}`:'',
...
function errorAlert(msg=null)
{
    swal.fire({
        ...
        text: msg === null?'Tuvimos un problema con los datos, por favor vuelva a intentarlo.  Si el problema persiste, contactese con un analista de sistemas.':msg,
        icon: 'error',
        showConfirmButton: false,
        timer: 2500
    })
...
function successAlert(response= null, isHTML = false)
{
    ...
    if(isHTML) {
        swal.fire({
            title: rflag?'Ocurrió un problema al realizar la operación':'Operación realizada con éxito',
            html: rflag?'':response,
